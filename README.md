...
그렇다면 26을 바로 최소로 연산하려고 구하지 말고, 1부터 최소 연산 횟수를 구해서, 그 다음 수에 대해서 최소연산 횟수를 구할 때 이전에 구한 수들의 최소 연산 횟수를 이용하는 바텀 업 방식의 다이나믹 프로그래밍 기법을 이용하면 될 것 같다. 이전에 계산했던 값들을 저장해 놨다가 사용하는 방식.
  ...
  **즉, 1을 뺏을 경우에 그 수를 만드는 최소 횟수 
  or 2 또는 3 또는 5 로 나누었을 때 그 수를 최소로 만드는 연산 횟수 + 1 (현재의 수 i를 만들기 위해 1을 더하거나, 2 또는 3 또는 5를 곱하는 연산횟수) 중 최소가 되는 값이 i를 1로 만드는 최소 연산 횟수**
  
  1부터 ~ n 까지 1을 만드는데 필요한 연산 횟수를 밑에서 부터 계산해 나가면 된다.
  
  1을 만드는 최소 연산 횟수 = 1
  
  2에 대해 적용할 수 있는 연산 = 2로 나누기 or 1 빼기 
  따라서 2를 만드는 최소 연산 횟수 =  (2 - 1)을 만드는 연산 횟수 or (2 / 1) 을 1로 만드는 연산 횟수 중 최소 값  + 1 ( 1을 빼거나 2를 나누는데 사용한 횟수 1 )
  
  3에 대해서 적용할 수 있는 연산 = 1 빼기 or 3으로 나누기
  따라서 3을 만드는 최소 연산 횟수 = (3 - 1)을 만드는 연산 횟수 or (3 / 1) 을 1로 만드는 연산 횟수 중 최소 값 + 1 ( 1을 빼거나 3을 나누는데 사용한 횟수 1 )
  ...
  # 앞서 계산된 결과를 저장하기 위한 DP 테이블 초기화
  dp = [0] * 30001  # 30,000번 인덱스를 사용하기 위해 30,001 크기로 리스트 생성
  
  # 다이나믹 프로그래밍 진행(바텀 업)
  
  for i in range(2, x + 1) : # X 번까지 진행
      
      # 현재의 수에서 1을 빼는 경우 = i-1을 구하는 횟수 + (1을 빼는 연산 + 1)
      dp[i] = dp[i-1] + 1
...
접근 :
식량 창고를 인접해서 털 수 없으므로, 최소 한 칸 이상 떨어 뜨려서 털어야 한다. 그렇다면 식량창고와 그 안의 식량이 리스트 형태로 주어졌을 경우,
0번째 인덱스 부터, n 번째 인덱스 까지 다이나믹 프로그래밍 기법을 이용해서 계산하면 될 것 같다.

따라서, n번째 창고 까지 털었을 때의 최대 값 = n-2번째 까지 털었을 때의 최대 값 + max(n-1번째를 안 털고, n번 째를 털었을 때 / n-1번째를 털고 n번째를 안털었을때)

이때, n = 1 일 때는 무조건 터는게 최대 값이고 -> dp[1] = data[1]

n = 2일 때는 => n = max(1을 털고 2를 안 털거나, 1을 안 털고 2를 털거나) = max(data[1] / data[2])

n = 3일 때는 => n = max(2를 털고 3을 안 털거나, 2를 안 털고 3을 털거나) = 1까지 털었을 때의 최고값 + max(2를 안터는 값 0 + 3을 터는 값 / 3을 터는 값 + 2를 안터는 값 0)

n 번째 일때 =>  n - 2 까지 털었을 때의 최고값 + max(n-1번째 값 / n 번째 값)

이 때 창고의 수 n은 최소 3 부터니깐, dp[3] 부터 기록해 나가면 될 것 같다.

dp[3] = dp[1] + max(data[2] , data[3])

2 1 2 1 

n 번째 까지 털었을 때의 최대 값 = ㅡmax(n-2를 털고 n을 털었을 때 값  vs n-1을 털었을 때의 최대 값)

1 까지 털었을 때의 최대 값 = 

3 번째 까지 털었을 때의 최대 값 = max(1까지 털었을 때 + 3 털었을 때, n-1까지 털었을때)

4 번째 까지 털었을 때의 최대 값 = max(2까지 털었을 때 최대값 + 4 털었을 때 값, 3까지 털었을 때 최대 값)


dp[n] = max(dp[n-2] + data[n], dp[n-1])
단, dp[2] = max(1을 안털고 2를 털 때, 1을 털고 2를 안 털 때) = max(data[1], data[2]) 
    dp[1] = data[1]
...
for i in range(3, n + 1) :
  dp[i] = max(dp[i-2] + list[i-1], dp[i-1])
...
접근 : 
세로의 길이 2는 고정, 가로의 길이 N만 1 ~ 1,000의 수를 가진다.
그렇다면 다이나믹 프로그래밍 기법을 이용해 이전에 계산했던 결과물을 저장하며 바텀업 방식으로 1 ~ N 길이 까지의 바닥을 채우는 방법의 수를 계산하면 될 것 같다.

먼저, 1칸의 크기를 채우는 방법 = 1가지
2칸의 크기를 채우는 방법 = 2가지

N = 1 길이 까지 채우는 방법 = 1가지
N = 2 길이 까지 채우는 방법 = 3가지

N 의 길이를 채우는 방법 = N-1 까지 채우는 방법 + N-2 까지 채우는고 큰거로 2칸 채우기 + N-2 까지 채우고 작은거로 2칸 채우기
= N - 1까지 채우는 방법 + N-2 까지 채우는 방법 X 2

예시 : 
N 을 채우는 경우의 수 = N - 1까지 채우고 1칸을 채우기 + N - 2 까지 채우고 2칸을 채우기 
이때, 1칸을 채우는 경우의 수는 1가지 밖에 없기 때문에 결국 N-1 까지 채우고 1칸을 채우기 = N-1칸 까지 채우는 경우의 수와 같다
N-2까지 채우고 2칸을 채우는 경우는 2칸을 채우는 경우의 수가 (큰거 하나, 작은거 2개로) 총 두 가지 이기 때문에,
N-2까지 채우는 경우의 수 + 큰거로 2칸을 채우는 경우 + N-2까지 채우는 경우의 수 + 작은거로 2칸을 채우는 경우 이렇게 총 2 개의 방법이 가능하다

따라서 N 길이 까지 채우는 방법의 수 = N-1 까지 채우는 방법 + N-2까지 채우고 큰 거로 2칸을 채우는 방법 + N-2까지 채우고 작은 거로 2칸을 채우는 방법
...
#### 다이나믹 프로그래밍 느낀점 :
다익스트라 알고리즘은 그리드 알고리즘으로 해답을 찾을 수 없는 경우, 완전 탐색과 비슷한 방법으로 모든 경우에 대해서 값을 구하지만, 단, 이전에 계산했던 값들을
저장하는 것을 다이나믹 프로그래밍이라고 한다. 이전에 이용된 값을 이용해서 다음 문제의 해답을 찾는 그런 문제해결 방법을 다이나믹 프로그래밍이라고 한다.
딱히 다이나믹 프로그래밍 문제라고해서 수학적인 점화식을 찾는 걸 의미하는게 아니라, 단지 이전에 계산된 값을 이용해서 다음 값을 찾는 규칙만 알아내면 된다.


### 가장 빠른 길 찾기
최단 경로(Shortest Path) 알고리즘은 말 그대로 가장 짧은 경로를 찾는 알고리즘이다. 그래서 ' 길 찾기' 문제 라고도 불린다. 최단 경로 알고리즘 유형에는 다양한 종류가 있는데, 상황에 맞는 효율적인 알고리즘이 이미 정립되어 있다.
...
최단 경로 알고리즘은 **보통 그래프**로 표현하는데 각 지점은 그래프에서 '노드'로 표현되고, 지점 간의 거리, 연결된 도로는 그래프에서 '간선'으로 표현된다. 또한 실제 코딩 테스트에서는 최단 경로를 모두 출력하는 문제보다는 단순히 최단 거리를 출력하도록 요구하는 문제가 많이 출제된다.

컴퓨터공학과 학부 수준에서 사용하는 최단 거리 알고리즘은 **'다익스트라 최단 경로 알고리즘', '플로이드 워셜', '벨만 포드'** 알고리즘 이렇게 3가지이다.
이 책에서는 이 중에 다익스트라 최단 경로와, 플로이드 워셜 알고리즘 유형만 다루려 한다. 이 2가지가 코딩 테스트에서 가장 많이 등장하는 유형이다.
따라서 이 유형만 파악해도 코딩 테스트 수준에서의 최단 경로 문제는 어렵지 않게 해결할 수 있다.
더불어 앞서 공부한 그리디 알고리즘과 다이나믹 프로그래밍 알고리즘이 최단 경로 알고리즘에 그대로 적용된다는 특징이 있다.
이번 장에서 배우는 내용은 사실 그리디 알고리즘 및 다이나믹 프로그래밍 알고리즘의 한 유형으로 볼 수 있다.

#### 다익스트라 최단 경로 알고리즘 - one to All
다익스트라 최단 경로 알고리즘은 그래프에서 여러개의 노드가 있을 때, 특정한 노드에서 출발하여 다른 노드로 가는 각각의 최단 경로를 구해주는 알고리즘이다. 다익스트라 최단 경로 알고리즘은 '음의 간선'이 없을 때 정상적으로 동작한다. 음의 간선이란 '마이너스' 값을 가지는 간선을 의미하는데,  현실 세계의 길은 음의 간선으로 표현되지 않으므로 다익스트라 알고리즘은 실제로 GPS 소프트웨어의 기본 알고리즘으로 채택되곤 한다.
다익스트라 최단 경로 알고리즘은 기본적으로 그리디 알고리즘으로 분류된다. 매번 **가장 비용이 적은 노드를 선택**해서 임의의 과정을 반복하기 때문이다.  알고리즘의 원리를 간략히
설명하면 다음과 같다.
...
다익스트라 알고리즘은 최단 경로를 구하는 과정에서 각 노드에 대한 현재까지의 최단 거리 정보를 항상 1차원 리스트에 저장하며 리스트를 계속 갱신 한다는 특징이 있다.
매번 현재 처리하고 있는 노드를 기준으로 주변 간선을 확인한다. 나중에 현재 처리하고 있는 노드와 인접한 노드로 도달하는 더 짧은 경로를 찾으면 '더 짧은 경로도 있었네?' 이제부터는 이 경로가 제일 짧은 경로야'라고 판단하는 것이다. 따라서 방문하지 않은 노드 중에서 현재 최단 거리가 가장 짧은 노드를 확인해 그 노드에 대하여 4번 과정을 수행한다는 점에서 그리디 알고리즘으로 볼 수 있다.
...
시험을 준비하는 사람은 방법 2를 정확히 이해하고 구현할 수 있을 때 까지 연습해야 한다.
특히 알고리즘 대회를 준비하는 독자라면 다익스트라 최단 경로 알고리즘은 자다가도 일어나서 바로 코드를 작성할 수 있을 정도로 코드에 숙달되어 있어야 한다. 또한 최단 경로 알고리즘을 응용해서 풀 수 있는 고난도 문제들이 많으므로 방법 2를 이해하고 정확히 구현할 수 있다면 다양한 고난이도 문제를 만났을 때도 도움을 얻을 수 있다.
...
그렇기 때문에, 사실 마지막 노드에 대해서는 해당 노드를 거쳐 다른 노드로 가는 경우를 확인할 필요가 없다. 예를 들어 위의 예시에서 이미 나머지 5개 노드에 대한 최단 거리가 확정된 상태이므로 더 이상 테이블이 갱신될 수 없기 때문이다. 

#### 방법 1. 간단한 다익스트라 알고리즘
간단한 다익스트라 알고리즘은 O(v <sup>2</sup>)의 시간복잡도를 가지며, 다익스트라에 의해서 처음 고안되었던 알고리즘이다. 여기서 V는 노드의 개수를 의미한다. 이 알고리즘은 직관적이고 쉽게 이해할 수 있다. 처음에 각 노드에 대한 최단 거리를 담는 1차원 리스트를 선언한다. 이후에 단계마다 '방문하지 않은 노드 중에서' 최단 거리가 가장 짧은 노드를 선택'하기 위해 매 단계마다 1차원 리스트의 모든 원소를 확인한다.

참고로 다음 소스코드에서는 입력되는 데이터의 수가 많다는 가정하에 파이썬 내장 함수인 input()을 더 빠르게 동작하는 sys.stdin.readline()으로 치환하여 사용하는 방법을 적용했다. 또한 DFS/BFS 에서의 소스코드와 마찬가지로 모든 리스트는 (노드의 + 1) 크기로 할당하여, 노드의 번호를 인덱스로 하여 바로 리스트에 접근할 수 있도록 했다. 그래프를 표현해야 할 때 많이 사용하는 일반적인 코드 작성법이므로 기억해두자.


```PYTHON
import sys
input = sys.stdin.readline   # sys.stdin.readline() 을 간편하게 쓰는 방법
INF = int(1e9) # 무한의 값을 표현하기 위한 수  10의 9승
...
# 각 노드에 연결되어 있는 노드에 대한 정보를 담는 리스트를 만들기 - 노드 연결 정보 리스트 생성
graph = [[] for i in range(n + 1) ]  # 노드의 개수 만큼 2차원 리스트를 생성, 리스트 안의 리스트

# 방문 여부 표시를 위한 리스트 만들기
visited = [False] * (n+1)

# 최단 거리 테이블을 모두 초기값인 무한 값으로 초기화
distance = [INF] * (n+1)

# 모든 간선 정보를 입력 받기 for
for _ in range(m) :
    from_node, to_node, cost = map(int, input().split())
    graph[from_node].append((to_node,cost))    # 불변하는 값은 튜플로 저장 to로 가는 비용 c는 불변
...
def get_smallest_node() :
   ...
   print("방문하지 않은 노드 중 최단 경로를 갖는 노드 탐색 중...\n")
   print("distance :", distance)
   print("visited :", visited)
   for i in range(1, n+1) :
       if distance[i] < min_value and not visited[i] : # 아직 방문하지 않았고 가장 최단 거리가 짧은 노드의 인덱스를 구함
            min_value = distance[i]
            index = i
            print("가장 짧은 거리를 갖는 노드를", index, "로 갱신")
   return index
...
def dijkstra(start) :
    # 시작 노드에 대해서 초기화
    distance[start] = 0   # 시작노드에서 시작 노드로의 거리는 0
    visited[start] = True  # 방문 처리

    for info_start in graph[start] : # start 노드의 모든 연결 정보를 가져옴
            distance[info_start[0]] = info_start[1]   # 시작 노드에서 갈 수 있는 노드와의 거리를 기록
            
    for i in range(n-1) : # 시작 노드를 제외하고, 아직 방문하지 않은 노드 중, 갈 수 있는 거리가 가장  짧은 최단 거리를 갖는 노드 타겟으로 계산하는 걸, 모든 노드에 대해서 반복해서 수행        
        # 현재 아직 방문하지 않았고, 갈 수 있는 노드 중 거리가 가장 짧은 최단 거리 노드를 꺼내서, 방문 처리
            now = get_smallest_node() 

            print("방문하지 않은 노드 중 최단 경로를 갖는 노드 :", now)
            visited[now] = True

            print("현재 노드 :", now)
            for info_next in graph[now] : # 최단 거리의 노드와 인접한 노드들의 연결 정보를 확인
                cost = distance[now] + info_next[1]  # now 노드 까지의 최소 거리 + 다음 노드 까지의 거리
                
                if cost < distance[info_next[0]]:
                    distance[info_next[0]] = cost     # 다음 노드 까지로 이동하는 더 짧은 경로를 발견한다면 갱신
